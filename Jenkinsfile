pipeline {
    ...
    environment {
        APP_NAME = "car-rental-payment-service"
        RELEASE = "1.0.0"
    ...
    stages {
        ...
        stage("Checkout Payment Service") {
            steps {
                git branch: 'main', credentialsId: 'GITHUB_LOGIN', url: 'https://github.com/myOrganization-GV/car-rental-payment-service'
            }
        }
        stage("Build Payment Service Application") {
            steps {
        ...
        stage("Build and Push Docker Image") {
            steps {
                script {
                    def jarPath = "target/car-rental-payment-service-0.0.1-SNAPSHOT.jar"
